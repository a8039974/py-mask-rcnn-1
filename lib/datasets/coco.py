...
class coco(imdb):
    ...
    def _load_coco_annotation(self, index):
        ...
        for obj in objs:
            ...
            if obj['area'] > 0 and x2 >= x1 and y2 >= y1:
                ...
                gt_masks = self._COCO.annToMask(obj)
                obj['clean_mask'] = gt_masks
        ...
        gt_masks = np.zeros((num_objs, width, height), dtype = bool)
    ...
    def _coco_results_one_category(self, boxes, segmentations, cat_id):
        results = []
        #boxes = detections[0]
        #print "boxes", boxes
        #masks = detections[1]
        for im_ind, index in enumerate(self.image_index):
            ...
            # dets = boxes[im_ind]
            dets_segmentations = segmentations[im_ind]
            #print "1...:",dets.shape
            #print "2...:",dets_masks.shape
            if dets == []:
                continue
            scores = dets[:, -1]
            xs = dets[:, 0]
            ...
            results.extend(
              [{'image_id' : index,
                ...
                'score' : scores[k],
                'segmentation' : dets_segmentations[k]}
                for k in xrange(dets.shape[0])])
        return results

    def _write_coco_results_file(self, all_boxes, all_segmentations, res_file):
        # [{"image_id": 42,
        #   "category_id": 18,
        ...
        for cls_ind, cls in enumerate(self.classes):
            ...
            #print "detections:",detections[cls_ind]
            #print "boxes:",all_boxes[cls_ind]
            #print "masks:",all_segmentations[cls_ind]
            results.extend(self._coco_results_one_category(all_boxes[cls_ind],all_segmentations[cls_ind],
                                                           coco_cat_id))
    ...
    def evaluate_detections(self, all_boxes, all_segmentations, output_dir):
        ...
        res_file += '.json'
        self._write_coco_results_file(all_boxes, all_segmentations, res_file)
